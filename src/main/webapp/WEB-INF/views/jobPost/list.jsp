         ...
         <div class="container">
            <a href="${contextPath}/index"
               class="navbar-brand d-flex align-items-center"> <svg
                  ...
                  <circle cx="12" cy="13" r="4" /></svg> <strong>Main</strong>
            </a>
         ...
         <div class="row py-lg-5">
            <div class="col-lg-6 col-md-8 mx-auto">
               ...
               <p class="lead text-body-secondary">Something short and leading
                  ...
                  it entirely.</p>
               <p>
                  <a href="#" class="btn btn-primary my-2">공고 보러가기</a> <a href="#"
                     class="btn btn-secondary my-2">이력서 작성</a>
               </p>
            </div>
         </div>
      </section>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="btn-group">
                        <a href="<c:url value='/board/view?bno=${post.jobPostId}'/>"
                            class="btn btn-sm btn-outline-secondary">View</a>
                        <!-- Add other buttons as needed -->
                    </div>
                    <!-- 스크랩 버튼 -->
                   <!-- 스크랩 버튼 -->
<div id="scrapButton-${post.jobPostId}">
    <c:choose>
        <c:when test="${not empty sessionScope.jobSeekerVo}">
            <!-- 세션에 로그인 정보가 있는 경우 -->
            <c:choose>
                <c:when test="${jobSeekerScrapVo.scrapId > 0}">
                    <i id="scrapIcon-${post.jobPostId}" class="fa-solid on-Clicked fa-star my-cursor" onclick="toggleScrap('${post.jobPostId}', '${jobSeekerScrapVo.scrapId}')"></i>
                </c:when>
                <c:otherwise>
                    <i id="scrapIcon-${post.jobPostId}" class="fa-regular fa-star my-cursor" onclick="toggleScrap('${post.jobPostId}', 0)"></i>
                </c:otherwise>
            </c:choose>
        </c:when>
        <c:otherwise>
            <!-- 세션에 로그인 정보가 없는 경우 -->
            <a href="<c:url value='/login'/>"> <i id="scrapIcon-${post.jobPostId}" class="fa-regular fa-star"></i> </a>
        </c:otherwise>
    </c:choose>
</div>
       ...
       $.ajax({
           ...
           success: function(response) {
               if (response === "canceled") {
                   $('#scrapIcon-' + jobPostId).removeClass('fa-solid on-Clicked').addClass('fa-regular');
                   alert('스크랩이 취소되었습니다.');
               } else {
                   $('#scrapIcon-' + jobPostId).removeClass('fa-regular').addClass('fa-solid on-Clicked');
                   alert('게시물이 스크랩되었습니다.');
               }
           },
